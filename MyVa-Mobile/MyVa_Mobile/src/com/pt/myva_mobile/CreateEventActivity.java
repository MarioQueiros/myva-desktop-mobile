...
public class CreateEventActivity extends Activity {
	...
	String[] eventsDesc;
	AutoCompleteTextView mEditTextDesc;
	Bitmap imageBitmap;
	int responseCode;
	Local local;
	Event event;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mEditText = (TextView) findViewById(R.id.editTextToShowLocal);
		mEditTextDesc = (AutoCompleteTextView) findViewById(R.id.editTextToShowDesc);
		mEditTextDesc.setAdapter(adapter1);
		...
		mEditTextDesc.setOnItemClickListener(new OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> p, View v, int pos, long id) {
				...
				in.hideSoftInputFromWindow(mEditTextDesc.getWindowToken(), 0);
			}
		});

		final View linearLayout = findViewById(R.id.create_event_id);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_map:
			if (isNetworkConnected()) {
				Intent j = new Intent(CreateEventActivity.this,
						MapActivity.class);
	...
	public void saveData() {
		...
		AutoCompleteTextView etName = (AutoCompleteTextView) findViewById(R.id.editTextToShowDesc);
		EditText etLocal = (EditText) findViewById(R.id.editTextToShowLocal);
		EditText etDate = (EditText) findViewById(R.id.editTextToShowDate);
		...
		if (save) {
			...
			event = new Event(etName.getText().toString(), (int) userId,
					local.getId(), (int) id_image, cal);

			long id = dbAdapter.getDBEventAdapter(getApplicationContext())
					.insertEvent(event);
			event.setId((int) id);
			...
			if (resultCode == RESULT_OK) {
				...
				if (!textMatchList.isEmpty()) {
					...
					if (textMatchList.get(0).contains("search")) {
						...
						EditText ed = (EditText) findViewById(R.id.editTextToShowDesc);
	...
	public void getMapResult(String msg) {
		TextView tv = (TextView) findViewById(R.id.editTextToShowLocal);
		if (!msg.equals("")) {
	...
	public void imageEditionListener() {
		ImageView imageEdition = (ImageView) findViewById(R.id.imageView5);
	...
	private class AddEventSync extends AsyncTask<String, Void, String> {
		String str_value = "";
		String str_value2 = "";
		long timestamp = 0;
		...
		private void postRequest() {
			...
			try {
				...
				request.setURI(new URI(Utils.SERVER_URL + "/addevent"));
				...
				HttpPost post = new HttpPost(Utils.SERVER_URL + "/addevent");
				...
				if (response.getStatusLine().getStatusCode() != 200) {
					...
					try {
						...
						str_value = json1.getString("AddEventID");
